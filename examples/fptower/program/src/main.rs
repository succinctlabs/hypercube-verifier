...
fn reduce_modulo(arr: &[u64; 6], modulus: &BigUint) -> [u64; 6] {
    let bigint = u64_6_to_biguint(arr);
...
pub fn main() {
    ...
    for _ in 0..1 {
        ...
        let a_bigint = u64_6_to_biguint(&a_reduced);
        let b_bigint = u64_6_to_biguint(&b_reduced);

        // Test addition
        assert_eq!(
            (a_bigint.clone() + b_bigint.clone()) % &modulus,
            u64_6_to_biguint(&add(&a_reduced, &b_reduced)) % &modulus
        );

        // Test addition with zero
        assert_eq!(
            (&a_bigint + &zero_bigint) % &modulus,
            u64_6_to_biguint(&add(&a_reduced, &zero)) % &modulus
        );
        ...
        assert_eq!(expected_sub, u64_6_to_biguint(&sub(&a_reduced, &b_reduced)) % &modulus);

        // Test subtraction with zero
        assert_eq!(
            (&a_bigint + &modulus - &zero_bigint) % &modulus,
            u64_6_to_biguint(&sub(&a_reduced, &zero)) % &modulus
        );

        // Test multiplication
        assert_eq!(
            (a_bigint.clone() * b_bigint.clone()) % &modulus,
            u64_6_to_biguint(&mul(&a_reduced, &b_reduced)) % &modulus
        );

        // Test multiplication with one
        assert_eq!(
            (&a_bigint * &one_bigint) % &modulus,
            u64_6_to_biguint(&mul(&a_reduced, &one)) % &modulus
        );

        // Test multiplication with zero
        assert_eq!(
            (&a_bigint * &zero_bigint) % &modulus,
            u64_6_to_biguint(&mul(&a_reduced, &zero)) % &modulus
        );
    }

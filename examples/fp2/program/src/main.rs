...
fn fp2_add(
    ...
    let mut lhs = [*lhs_c0, *lhs_c1].concat();
    let rhs = [*rhs_c0, *rhs_c1].concat();

    syscall_bls12381_fp2_addmod(lhs.as_mut_ptr(), rhs.as_ptr());

    let result_c0: [u64; 6] = lhs[0..6].try_into().unwrap();
    let result_c1: [u64; 6] = lhs[6..12].try_into().unwrap();
...
pub fn main() {
    ...
    for _ in 0..10 {
        ...
        let c1 = ((a_c0_bigint * b_c1_bigint) % &modulus + (a_c1_bigint * b_c0_bigint) % &modulus)
            % &modulus;

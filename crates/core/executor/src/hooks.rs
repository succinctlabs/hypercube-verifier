...
#[must_use]
pub(crate) fn hook_ecrecover(buf: &[u8]) -> Vec<Vec<u8>> {
...
pub(crate) mod fp_ops {
    use super::{pad_to_be, BigUint, One, Zero};
    ...
    pub fn hook_fp_inverse(buf: &[u8]) -> Vec<Vec<u8>> {
    ...
    /// - If the element is zero.
    pub fn hook_fp_sqrt(buf: &[u8]) -> Vec<Vec<u8>> {
        let len: usize = u32::from_be_bytes(buf[0..4].try_into().unwrap()) as usize;

        assert!(
            buf.len() == 4 + 3 * len,
            "FpOp: Invalid buffer length, found: {}, expected: {}",
            buf.len(),
            4 + 3 * len
        );
...
#[must_use]
pub(crate) fn hook_ed_decompress(buf: &[u8]) -> Vec<Vec<u8>> {
...
pub(crate) mod bls {
    ...
    pub fn hook_bls12_381_sqrt(buf: &[u8]) -> Vec<Vec<u8>> {
    ...
    pub fn hook_bls12_381_inverse(buf: &[u8]) -> Vec<Vec<u8>> {
        let field_element = BigUint::from_bytes_be(&buf[..48]);
...
#[must_use]
pub(crate) fn hook_rsa_mul_mod(buf: &[u8]) -> Vec<Vec<u8>> {
    assert!(
